...
`orpah-over-halow/energy.py`；其中的参数**默认是演示标定值**，可用**实测标定文件**替换
（`orpah-over-halow/energy_calib.py`，路径由 `ORPAH_ENERGY_CALIB` 指定；逐项标出处
`measured`/`demo`，文件有问题则**整份不采用**回退到演示值 —— **不许半份生效**）。
真机标定数值本身仍未做，见 F-11。规范只要求上面的 E1/E2/E3 成立，**不**规定 P/C/cost 的数值。
...
| F-11 | 免电池终端的能量预算与状态机（采集功率/储能/上报代价的**真实标定**，以及“能量紧到什么程度该降级/该沉默”的**阈值**），是否要写进《Orpah ID 协议规范》§8.1 的“条件”列成为一条正式降级理由？ | §5.2/《Orpah ID 协议规范》§8.1/§9 | **部分已定（2026-09-13）**：机制与语义已定并实现（§5.2 的 E1/E2/E3：降级可见、下限 L1、沉默归因分叉；`energy.py` + 告警 + UI）；**实测标定入口已实现**（`energy_calib.py`：JSON 文件 / `ORPAH_ENERGY_CALIB`，可写原始实测或 mW/mJ，逐项标 `measured`/`demo` + 溯源与文件指纹，**坏文件整份不采用**；阈值等**策略项不是标定项**）；**待定 = 真机标定数值本身**（TX-AH+CH32V203 实测采集/功耗）与是否把“能量”列入《Orpah ID 协议规范》§8.1 条件列 —— **没标定时一切仍为演示值（页面徐标会写“未标定”，不得当成实测）** |
