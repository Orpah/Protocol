...
- 状态：**v0.7.6**（2026-09-13）· L1/L2 已在 `orpah-over-halow` 实现并验收
...
- **已定方向（2026-09-13 用户定，**取代**原「受信 Router 集合」建议）**：按原则 P-1 做
  **多观测一致性判定**，`xport` **永不单独采信**：
  1. 用**任意子集**的观测解位置 → 反推每台 Router **应有的 RSSI** → 比较实测残差；
  2. 残差离群的那台**被剔除**（最小中位数 / RANSAC 式），并作为**安全事件**记录
     （有攻击 = 有线索，这是重要信号，不能静默丢弃）；
