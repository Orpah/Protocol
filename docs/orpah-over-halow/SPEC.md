...
- 状态：**v0.7.9**（2026-09-13）· L1/L2 已在 `orpah-over-halow` 实现并验收
  ...
  **z ≥ 5**：实测诚实场景 max-z 的上界 ≈ **3.8 且与噪声幅度无关**（±1/±2/±4 dBm 同样），
  故 3 会误报（实测诚实满网格 11% 被冤枉）、5 则不误报（**5 千多个诚实样本零误报、零“残差偏大”误报**）。
  同时量清了**三条能力边界**（全部写进 §8 威胁 2）：
  ① **“谎报更近”在数学上不可分辨** —— 它的误差上限就是它自己报的距离，z 有上界（实测最大 ≈3.8，
     与诚实尾部重合）→ 无论怎么调阈值都拓不到（×0.25/×0.1 实测 **864/864 全漏**，如实不报，
     不编“已防住”）；② “谎报更远”无上界 → 可靠可检（4 台 77%、5 台 87%；噪声 ±4 dBm 时 4 台降到 60%）；
  ③ **多台同时谎报**（n=4 里 2 台）属信息论边界（n−k≥3 才能定位）→ 定不了是谁，
     但**拟合残差 σ0 会变大**（诚实 0.2~0.8 vs 2 台谎报 1.5~2.2）→ 页面按实测阀值 **1.8** 报
     「拟合偏松」这一**事实**（不指认谁、不保证每帧都报）。
  另一处实测修正：z 的分母必须含**预测的不确定度**（`wlsLocate` 协方差沿“预测点→本台”方向的投影
  ⊕ σ0 放大）—— 只看本台测距噪声时，其余台里混着撒谎的会把**诚实台**算出大 z（实测诚实台 z=9.3
  与真撒谎那台 12.0 分不开）→ 误剔除 11%（其中路线区域 28%）；补上后归零。
...
- **已定方向（2026-09-13 用户定，**取代**原「受信 Router 集合」建议；**同日已实现**于
  ...
  1. **判据 = 留一法标准化残差** `z_i`：用**其余台**解出位置，再看这一台的实测距离"应该是多少"：
     ...
     才定案 → 剔除 + 作为**安全事件**记录（有攻击 = 有线索，不能静默丢弃）。
     **阈值 = z ≥ 5（按实测零假设分布标定）**：诚实场景 max-z 上界 ≈ **3.8，且与噪声幅度无关**
     （统计量已对噪声归一）；早先拍的 3 会误报（实测诚实满网格 11% 被冤枉）→ 改 5 后
     **5 千多诚实样本零误报**。同时要求「**明显突出**」（最大 z ≥ 1.5× 次高）：其余台里混着撒谎那台时，
     诚实台的 z 也会偏大（z_S4=9.3 vs 真撒谎 12.0 几乎分不开）—— 要冤枉一台得同时满足“超 5σ”且“高出很多”。
     另：z 的分母 = 本台测距噪声 ⊕ **预测的不确定度**（预测点协方差沿“预测点→本台”方向的投影，
     再用 σ0 放大）—— 少了这一项，诚实台会被冤枉（见下）。两条实测踩出来的「**别这么写**」：
     - **不要用「单台残差 > k·σ_i」**（对**整体拟合**求残差）：单个离群观测会把拟合**带弯**，
       各台残差被摊平到各自阈值以内 —— 4 台里 1 台把距离报成 1/4，那一判据下**全部合格**（漏检）。
     - **σ 不能锚在它自己报的距离上**，必须锚在**其余台推出的**距离上：锚错时，谎报「我在更远处」
       会把自己的 σ 一起放大（`z = 4|f−1|/f`，**封顶 4.0**）→ **谎报更远永远抓不住**：实测 ×4 谎报
       在整条路线上 **0/120 全没发现**，还照样报「交叉校验通过」。
     - **分母不能只算本台测距噪声**（不把预测的不确定度算进去）：其余台里混着一台撒谎的时，
       预测点本身就被带偏，于是**诚实的那台也会算出很大的 z**（实测 z_S4 = 9.3 与真正撒谎的 12.0
       几乎分不开）→ **诚实场景 11% 被冤枉剔除**（路线区域 28%），而一台谎报时六成判不出。
       补上「预测协方差投影 ⊕ σ0 放大」后，诚实误剔除 **0**。
     （**也不要**把「排除它后整体是否自洽」当唯一判据：4 台剔到 3 台时 dof=1、χ² 上限很松，
     连排除**诚实**那台都能算出「自洽」→ 定不了谁。）
  2. **移动目标**：用滤波状态预测**各观测各自时刻**的位置再算残差（复用 `pos.js` 的恒速卡尔曼，
     仍是**不看未来**）—— 否则正常走动会被误判成「被篡改」（**假警报最害人**）。
...
- **硬边界（必须如实遵守，不许包装成「防住了」）**：
  ...
  - **辨识性需要 ≥4 台**：3 台里有 1 台不一致时，**测得出「不一致」但判不出是谁**
    （排除一台只剩 2 台：要么拟合得天衣无缝、要么两圆根本不相交 —— 都不能定谁，硬指就是冤枉）
    → 只能报「观测冲突」+ 参考线索（`suspects`，**仅线索不是结论**）。
    **要识别单台离群，实网需 ≥4 台站位**（这是实现里测出来的部署要求）。
  - **k 台合谋不可防**（信息论边界：剩 n−k 台，要 n−k ≥ 3 才还能定位）。**具体到 n=4**：
    2 台同时谎报（4−2=2 < 3）→ **定不了是谁**（实测：各台 z 都变小、判不出）；
    此时退回**唯一还能用的事实**——**拟合残差 σ0**：诚实 0.2~0.8 vs 2 台谎报 1.5~2.2（跨噪声稳定），
    页面按 **1.8** 报「拟合偏松」（**只陈述事实、不指认谁**，单帧可能压线不报）。
  - **能抓与抓不住是【不对称】的（实测，必须如实写，别包装成“对称可检”）**：
    - **谎报“更远”（f>1）无上界 → 可靠可检**：z ∝ (f−1) 随 f 线性涨。实测（1 台谎报 ×4）：
      4 台 **77%** 剔除、5 台 **87%**、±4 dBm 时 4 台降到 **60%**（其余如实报冲突/不报）。
    - **谎报“更近”（f<1）在数学上不可分辨**：它的误差上限就是它自己报的距离 →
      z 有硬上界（实测最大 ≈3.8，与**诚实尾部重合**）→ **无论怎么调阈值都拓不到**；
      实测 ×0.25 / ×0.1 **864/864 全漏**（如实不报）。这恰恰是最有威胁的方向
      （报近 → σ 小 → 权重高 → 把定位往它那边拉）→ **空间一致性拦不住，需要时间/多帧一致性
      或其他无关通道**（未实现，见 F-12）。
