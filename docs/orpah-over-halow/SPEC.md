...
按 §8.3 只能做**覆盖发现（coverage-only）**，**不算人员出现**；若允许能量导致 L3，则“人到了
...
| F-11 | 免电池终端的能量预算与状态机（采集功率/储能/上报代价的**真实标定**，以及“能量紧到什么程度该降级/该沉默”的**阈值**），是否要写进 §8.1 的“条件”列成为一条正式降级理由？ | §5.2/§8.1/§9 | **部分已定（2026-09-13）**：机制与语义已定并实现（§5.2 的 E1/E2/E3：降级可见、下限 L1、沉默归因分叉；`energy.py` + 告警 + UI）；**待定 = 真机标定数值**（TX-AH+CH32V203 实测采集/功耗）与是否把“能量”列入 §8.1 条件列（现为**演示值、非实测**） |
