...
真机标定数值本身仍未做，见 F-11。规范只要求上面的 E1/E2/E3 成立，**不**规定这些参数的数值。
...
- **L2.6（能量轴，✅ 已完成 2026-09-13）**：免电池客户端的能量模型（采集 P / 储能 C / 每次上报
  ...
  **「没电了」从沉默里分流出来**；规范见 §5.2，实现 `orpah-over-halow/energy.py`（+ `alerts.py` 新 kind
  `id_energy` / `no_report_energy`、服务端成因推导），UI 「能量轴」卡片带采集→间隔扫描表。
  参数默认是演示标定值；**实测标定入口已实现**（`energy_calib.py`：读文件 → 换算 →
  逐项标出处，见 §5.2 非规范段）。**真机标定数值（TX-AH+CH32V203 实测）仍未做，见 F-11**。
...
| F-11 | 免电池终端的能量预算与状态机（采集功率/储能/上报代价/**监听开销**的**真实标定**，以及“能量紧到什么程度该降级/该沉默”的**阈值**），是否要写进《Orpah ID 协议规范》§8.1 的“条件”列成为一条正式降级理由？ | §5.2/《Orpah ID 协议规范》§8.1/§9 | **部分已定（2026-09-13）**：机制与语义已定并实现（§5.2 的 E1/E2/E3：降级可见、下限 L1、沉默归因分叉；**监听作为固定开销进预算**，采集连监听都供不上时如实沉默、`short_of` 指明缺在哪一层；`energy.py` + 告警 + UI）；**实测标定入口已实现**（`energy_calib.py`：JSON 文件 / `ORPAH_ENERGY_CALIB`，可写原始实测或 mW/mJ，逐项标 `measured`/`demo` + 溯源与文件指纹，**坏文件整份不采用**；阈值与**听间隔**等**策略项不是标定项**）；**待定 = 真机标定数值本身**（TX-AH+CH32V203 实测采集/待机/上报/听窗口）与是否把“能量”列入《Orpah ID 协议规范》§8.1 条件列 —— **没标定时一切仍为演示值（页面徽标会写“未标定”，不得当成实测）** |
