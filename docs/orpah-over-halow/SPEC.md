...
- 状态：**v0.7.15**（2026-09-13）· L1/L2 已在 `orpah-over-halow` 实现并验收
  ...
  **末次（v0.7.15）：新增威胁 4 —— Router 的「下行」没真实性**（LOST-TABLE/回执/ERROR 都未签名，
  一条伪造的空表能改走失缓存 + 让 `_synced=True` → 此后不再拉表、命中也不答、不再产生 FOUND）。
  **A 方案已实现**（下行只收配置的 Server 源 IP+源端口，解析前就丢 + 计数可见，fail-closed）；
  **B 方案（下行带 HMAC/签名）未做**，记为 **F-14**（需与密钥分发一起定，与 CRL 分发同批）。
  验收：`test_router.py` 9 项（离线）+ `demo_l4.py` 真 UDP 的“假表被丢/缓存不动/真表生效”三条。
- 文档负责人：shijh（Orpah）
...
- **仍未做（B 方案，需与真机/密钥分发一起定）**：**源地址校验 ≠ 真实性**（同源也能被伪造；
...
- **L3（真机前续，规划）**：免电池客户端(TX-AH+CH32V203)低功耗策略；F-05 防伪造/限频、
  ...
  真机侧剩：RSSI/能量**实测标定**（F-11）、**下行真实性 B 方案（F-14，需与密钥分发一起定）**。
...
| F-14 | **Router 的「下行」（Server→Router）没有真实性**（LOST-TABLE/回执/ERROR 均未签名）—— 怎么办？ | §8 威胁 4 | **A 已实现（2026-09-13 用户选）**：下行**只接受配置的 Server 源 IP + 源端口**（`router._from_server`，解析之前就丢 + `down_rejected` 计数/留痕 + 页面可见；`server_host` 为主机名时先解析）。**fail-closed**：真机若源地址与配置不同（NAT/多宿主）会表现为“下行全被丢”，显式失败而非默默信任。验收 `test_router.py` 9 项 + `demo_l4.py` 真 UDP 三条。**B 未做**：下行带 HMAC/签名（Router 持共享密钥或 Server 公钥）—— **需与密钥分发一起定**，与「CRL 分发到 Router」（F-10）属同一批 Router 侧信任问题；**源地址校验 ≠ 真实性**（同源也能伪造），不得写成“已防住” |
