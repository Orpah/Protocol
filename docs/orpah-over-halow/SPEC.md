...
- 状态：**v0.7.18**（2026-09-14）· L1/L2 已在 `orpah-over-halow` 实现并验收
  ...
  **末次（v0.7.16）：F-14 的 B 方案也实现了 —— 下行带 ES256 签名**（2026-09-13）。
  下行新增三字段：`dts`（Server 签名时刻）/ `dn`（Server 单调计数）/ `dsig`（`base64url(ES256(raw r||s))`），
  预像 = `JCS(报文去掉 dsig)`（整条报文盖住）；Server 在 `_reply`（**下行唯一出口**）签，
  Router 在**来源校验之后、动作之前**验，并要求 `|now−dts| ≤ ORPAH_DOWN_WINDOW`（默认 300s）
  且 `(dts,dn)` 比上次已接受的那对更大（**防重放** —— 重放不需要私钥）。
  **为何不用 HMAC 共享密钥**：Router 本就假定可被改装，共享密钥进 Router 等于把伪造能力交给改机的人；
  公钥不是秘密。**边界照实**：空口那段（Router→Client）仍无认证；Router 未配公钥时
  **计数 `down_unverified` + 页面显示“未启用”**（不假装已启用）；私钥硬件信任根、TLS/持久会话未做。
  ...
  **它不是防线**（被改的设备不做），语义是**延后而非丢弃**，验收 `test_selflimit.py` 24 项 +
  ...
  同节奏绕过自限频的对照 → 上游丢 16）。
  **又续（v0.7.18，2026-09-14）：覆盖（E4）收口 + “设计常态 60 s”定为基准。**
  ① E4③ 落实：新告警 kind `id_cover_short`（`degrade` warn / `short` crit）—— 判据**直接取模型
  `verdict`**（不重算）、要求该 SN 有**已签**电量、`ok`/`none` 不报；断线**之前**就能看到，
  红点/通知/跳转齐全（实测：缺口 12h → 该条带「能撑多久 / 还差多少 / 需约 21.6 J 储能 /
  降级换覆盖后可撑 3h20m」，缺口改 1h → 自动消警）。
  ② **设计常态 = 客户端每 60 s 连一次 HaLow Router**（用户原话；连接时顺带听下行 → 与听间隔同值）
  写成**设计基准**（§5.2 末段“非规范（设计基准）”）：实现里 `NORMAL_INTERVAL_S` 是**单一源**
  （CLI 默认 / 装配默认 / 页面默认 / 告警阀值四处都指向它），**“还跟得住”（≤ 300 s 底线）与
  “在常态”分开描述**（四档 `ok`/`slower`/`too_slow`/`silent`），告警“多久没上报”阀值改为
  **按节拍成比例**（`2.5×`/`5×` = 150 s / 300 s，原来拍的 30 s 是拿 2 s 演示周期算的）。
  低采集下**保持签名强度、如实标已降速**（不自行降级），同时把“降级能否换回常态”摆出来供取舍。
  验收：`test_energy.py` 第 14 节 + `test_alerts.py`（阀值比例锁）+ `test_server.py::TestEnergyDesignNormal`。
  **如实记一笔（同日已收口）**：常态 60 s 下 `rssi_jump`（原窗口只在 0.5~15 s 的相邻样本间比）
  本**不会自然触发** —— 已**按常态重新标定**：实测诚实序列（dt 1 s~1800 s、168 h、816803 对）
  跳变上界 **19 dB** 且**只在 1→30 s 增长后饱和**（序列是“最近那台 Router 的链路强度”，几何自归一）→
  只改**窗口**为 **0.5~450 s**（= 1.5 × 最长合法上报间隔），**阀值 25 dB 不变**（依旧零误报）。
- 文档负责人：shijh（Orpah）
...
**E4（SHOULD，部署要求，2026-09-13 用户定）**：**不得把“取能随环境波动”当成停机的理由。**
取能功率波动是**换能器的事**（光/运动/温差），而 ORPAH 是找人 —— **夜间/取能低谷恰恰是必须
覆盖的时段**（黄金时间含夜；夜里更危险）。因此：① 系统应按**最坏无取能时段**与实测取能曲线
核算**覆盖**（能撑多久、要多少储能），而不是把“天黑就不工作”当成正常循环；② 覆盖不足时
**首先降级换覆盖**（降到 HS256 + 把上报间隔拉到可用上限，宁可弱一点/慢一点也**不要沉默**），
并把结果与还差多少报出去；③ **断线前的预警与断线事件必须可见**（它是**设计不足**，属于要处置的
告警，不是作息）；④ 预期会断线时，应在**断电之前**如实上报自己的预测（见 E3 的沉默归因）。
演示实现与实测数字见 §9 L2.6、`orpah-over-halow/energy.coverage()`。
**实现状态（如实）**：①② 已实现（模型 + 页面）；③ 已实现（新告警 kind `id_cover_short`：
`degrade` warn / `short` crit，判据取模型 `verdict`、要求该 SN 有已签电量）；④ **未做** —— 它需要
**设备侧**在断电前把预测发出来，而报文里目前**没有**这个字段（**加字段 = 协议变更**，未对齐前不动）。

**非规范（实现选择，可变）**：具体“采集→间隔→级别”的映射用储能模型实现于
`orpah-over-halow/energy.py`：采集 P（mW）/ 储能 C（mJ）/ 每次上报代价 cost（mJ）/
**每次听窗口耗电 × 多久听一次**（监听 = 下行带来的**固定开销**，与上报间隔无关；
真正能拿去上报的是 `采集 − 待机 − 监听`）。`listen_interval_s` 属**产品选择**（不是标定项：
听间隔↑ = 下行变慢、发现更慢），且采集连监听都供不上时实现**只如实沉默**、
**不会自动拉长听间隔**。参数**默认是演示标定值**，可用**实测标定文件**替换
（`orpah-over-halow/energy_calib.py`，路径由 `ORPAH_ENERGY_CALIB` 指定；逐项标出处
`measured`/`demo`，文件有问题则**整份不采用**回退到演示值 —— **不许半份生效**）。
真机标定数值本身仍未做，见 F-11。规范只要求上面的 E1/E2/E3/E4 成立，**不**规定这些参数的数值。

**非规范（设计基准，2026-09-14 用户定）**：**设计常态 = 客户端每 60 s 连一次 HaLow Router**
（连接时顺带把下行听了 → 与“听间隔”同值）。它是个**基准**，不是可选档位：
① 规范里的“多久没上报才算异常”（现为 `2.5×` / `5×` 常态周期 = 150 s / 300 s）
**随这个节拍成比例推**，而不是拍一个绝对秒数；② **“还跟得住”（≤ 可用上限 300 s，**底线**）
与“在常态”是两件事**，不得混说 —— 实现用三档 `ok`/`slower`/`too_slow`（+ 无间隔时的 `silent`）
表达；③ 采集不足以**同时**支撑强签名与常态时，实现**保持签名强度、如实标已降速**，
**不自行降级**，但要把“降级能否换回常态”摆出来供人取舍（签名强度 vs 更新频率是产品选择）。
演示实现与数字见 `orpah-over-halow/energy.py`（`NORMAL_INTERVAL_S`）、§9 L2.6、F-11。
...
- **硬边界（必须如实遵守，不许包装成「防住了」）**：
  ...
  - **能抓与抓不住是【不对称】的（实测，必须如实写，别包装成“对称可检”）**：
    ...
    - **“偏得更近”（f<1）在数学上不可分辨**：它的误差上限就是它自己报的距离 →
      ...
      （报得近 → σ 小 → 权重高 → 把定位往它那边拉）→ **空间（单帧）一致性拦不住**，
      靠**多帧（时间）持续性**补（见下一条，2026-09-13 已实现）。
  - **多帧（时序）持续偏差扫描（`biasScan`，2026-09-13 已实现）= 拓“单帧不可分辨”那一侧的补充**：
    逐帧算「实测 / 其余台推出的预测」比值，再看各台的**中位比值**是否长期偏离 1。
    诚实台的中位比值趋 1；持续偏 k 倍的台趋 k，帧数一多就分得开（比值已对噪声归一）。
    阈值同样**按实测零假设分布**定（4 台站位 + 匀速靶标 + ±2 dBm 噪声，每档 40 组种子）：
    诚实 z 上界 —— **60 帧 7.4 / 100 帧 5.8 / 300 帧 6.7 / 800 帧 18.5 / 2000 帧 22.2**
    （**随帧数缓慢上涨**：比值序列时间相关，`MAD/√n` 低估不确定度；但上涨次线性）→ **阈值取 30**，
    帧数 >2000 就均匀抽稀。检出下限（实测，取最坏种子）：**×0.5 / ×0.25 从 ~100 帧起可靠**
    （z 最小 31.8 / 93.6），**×0.75 要 ≳300 帧**（31.4），60 帧连 ×0.75 都分不开（5.3）。
    **三条边界必须与结果一起呈现**：① 帧数越少越不灵；② 它比的是“这台 vs 其余台”，
    **全体一起偏（全局标定偏移）它看不出来** → 全局性偏差靠实测标定（F-11）；
    ③ **比值偏离 ≠ 该台有问题** —— 该台落在其余台几何之外时，留一法的预测本身有系统外推偏差
    （演示里 S1 实测中位 0.90）→ 所以它**只能当线索、必须靠别的独立来源核实**（孤证不立的又一例）。
    实现上必须**两步**：第一遍会把**被真凶拖累的台**一起标出（实测：把一台改成持续 ×0.25，
    真凶 z=588，另两台被拖到 47 / 32 也越线）→ 必须**剔掉最狠那台再扫一遍**：
    其余台回到 1 附近（实测 z 3.6）才敢说“最像是就它”；若剔除后仍有台偏 → 如实说
    **“定不了是哪台”**（多台同时偏、或几何/标定问题），这与单帧判定“剔除后再验”是同一套思路。
...
**威胁 4（本链路特有，2026-09-13，F-14）：Router 的「下行」（Server → Router）没有真实性**

- 事实：Router 收下的三类下行报文 —— **LOST-TABLE**、**TRACKING-STATUS**、**ERROR** ——
  **都没有签名**，`router._udp_loop` 只按报文类型分派。
- 后果链（不是“计数器不准”那种量级）：
  1. 任何**能向该 Router 的 UDP 端口发包**的人可以塞一张**伪造的 LOST-TABLE**；
  2. `_apply_lost_table` 会**整体替换走失缓存**，并置 `_lost_event` → `sync()` 返回 True →
     **`_synced = True`**；
  3. 此后 REQ-CONNECT 的“未同步就拉表”分支被跳过，**一直用这张假表** → 真实走失者被答成
     `NOT-TRACKED`、**不再产生 ORPAH-FOUND**（即：**一条伪造的空表就能“弄瞎”这台 Router**，
     攻击者还可以持续重发；最阴的版本是只把某个 SN 从 `entries` 里摸掉 —— 其它一切正常，
     只有那一个人不再被发现）。
  4. 同理可伪造回执（TRACKING-STATUS）与错误报文，并让 Router 注入回空口。
- **已实现（A 方案，2026-09-13 用户选）**：`router._from_server()` —— 下行**只接受来自配置的
  Server 地址（源 IP + 源端口）**的报文，别的**在解析之前**就丢，并计数/留痕
  （`down_rejected` / `down_rejects`，`/api/status.router_down_rejected`，页面 Router 行可见）。
  `server_host` 写主机名时先解析一次（`localhost → 127.0.0.1`）。
  **fail-closed**：真机部署若 Server 回包源地址与配置不同（NAT/多宿主/改过 `server_host`），
  表现为“下行全被丢”并有日志 —— 显式失败，不默默信任。
  验收：`test_router.py`（9 项，离线）+ `demo_l4.py` 第 ④ 组（**真 UDP**：从 `127.0.0.2`
  打一张假表进去 → 被丢且缓存不动；对照组 Server 来的照常生效）。
- **✅ 已实现（B 方案，2026-09-13 完成）**：下行带 **ES256 签名**（`downlink.py` / `server._reply` / `router._check_down_sig`）。
  - **字段（下行报文新增，三类下行都有）**：`dts`（Server 签名时刻，epoch 秒）、
    `dn`（Server 侧单调计数）、`dsig` = `base64url(ES256(raw r||s))`；
    **签名预像 = `JCS(报文去掉 dsig)`** —— entries / rid / status / sn 全盖住，改任一字段即验不过。
  - **谁签谁验**：Server 在 `_reply`（**下行唯一出口**）签，Router 在**来源校验之后、动作之前**验
    （无签名 `no_sig` / 签名错 `bad_sig` / 超窗 `stale` / 重放 `replay` 全拒，fail-closed）。
  - **为什么是非对称（而不是 HMAC 共享密钥）**：本链路的 Router 本就假定**可被改装**
    （见 §8 原则 P-1 与威胁 2）—— 共享密钥一旦放进 Router，**改一台就拿到给别的 Router 发假表的密钥**；
    公钥不是秘密，改装一台学不到伪造能力。代价是下行多一次 ECDSA 验签（下行条数很少）。
  - **新鲜性必须带，否则等于没修**：只签名不防重放时，**重放一张旧空表**照样弄瞎 Router
    （重放**不需要私钥**）→ Router 要求 `|now − dts| ≤ 300 s` 且 `(dts, dn)` **大于**上次已接受的那对
    （按报文类型各记）。Server 重启 `dn` 归零不会卡死（`dts` 更大即可通过）。
  - **两条边界照实**：① Router **没配公钥**时无法校验：照旧收下（不把 demo 搞挂）但**计数
    `down_unverified`** + 日志提示 + 页面显示「未启用（只有来源校验）」，**不得**当成已启用；
    ② 本方案只管 **Server → Router**；**空口那一段（Router → Client）仍无认证**
    （低功耗客户端验不了签名，空口开放是本项目前提）→ 能往空口丢帧的人照样能骗客户端。
  - 其它边界：私钥保管不在本方案范围（demo 是 PEM 文件，真机需硬件信任根/权限隔离）；
    能长期在线路上做中间人的人仍可**丢弃/延迟**合法下行（可用性），也能在窗口内重放**更新**的那一条 ——
    要挡这些得靠 TLS/持久会话（未做）。
  - 配置：`ORPAH_DOWN_WINDOW`（默认 300）/ `ORPAH_DOWN_PUB`（Router 读公钥 PEM；
    `ui_server` 启动时把公钥写到这里，缺省系统临时目录下固定名）/ `ORPAH_DOWN_KEY`（Server 读私钥，
    不设则现场生成一把）。
  - 验收：`test_downlink.py`（44 项）+ `test_router.py` + `test_server.py` + `demo_l4.py` 第 ⑨ 组
    （真 UDP：**同源套接字**（IP+端口都与 Server 相同，A 根本认不出）的无签名空表被丢、缓存一动没动；
    伪造签名被丢；重放被丢）。
- **仍未做**：下行**重试/持久会话**（TLS，挡“中间人丢包/延迟”）、私钥的**硬件信任根**。
  ★ **源地址校验 ≠ 真实性**：A 单独用时不得写成“已防住”（同源可伪造、跳NAT 会 fail-closed）；
  A + B 两者一起才谈得上“下行有真实性（除上述边界）”。
...
- **L2.6（能量轴，✅ 已完成 2026-09-13）**：免电池客户端的能量模型（采集 P / 储能 C / 每次上报
  代价 cost / **每次听窗口耗电 + 听间隔**）+ 由能量驱动的**间隔与降级**（下限 L1）+
  **「没电了」从沉默里分流出来** + **覆盖（不断线，见 §5.2 E4）**；规范见 §5.2，
  实现 `orpah-over-halow/energy.py`（`plan` / `coverage`；+ `alerts.py` 新 kind
  `id_energy` / `no_report_energy`、服务端成因推导），UI 「能量轴」卡片带采集→间隔扫描表
  与覆盖块（缺口时长 / 实测曲线）。参数默认是演示标定值；**实测标定入口已实现**
  （`energy_calib.py`：读文件 → 换算 → 逐项标出处，含可选的**实测取能曲线**，见 §5.2 非规范段）。
  ★ **实测曲线有两种口径**（2026-09-22 补，非规范段口径澄清）：`profile` = **一段实测窗口**
  （可非周期，**不假设重复**）—— 只回答“这段窗口会不会断、什么时候断、撑过它要多少储能”，
  **不谈“可永续”**；`period` = **一个典型周期**（假定周期重复）—— 才谈周期净收支与可永续。
  同一份数据两种口径的**数值结论相同**，但“能不能叫永续”不同 ⇒ 口径由**数据来源**决定，
  不得猜、不得互相替代；页面/告警按它换标签（缺口 / 实测窗口 / 典型周期）。
  **设计常态 = 每 60 s 连接一次**（2026-09-14 用户定，见 §5.2 末段“设计基准”）：实现里
  `NORMAL_INTERVAL_S` 是单一源（CLI/装配/页面/告警阈值四处都指向它），常态与“还跟得住”分开显示，
  告警的“多久没上报”阈值按它成比例（150 s / 300 s）。
  **真机标定数值（TX-AH+CH32V203 实测）仍未做，见 F-11**。
...
- **L3（真机前续，规划）**：免电池客户端(TX-AH+CH32V203)低功耗策略；F-05 防伪造/限频、
  ...
  其中**限频 §5.8 四行已全部实现**，见 §8 威胁 1）、**`xport` 信任问题升格为 F-12**）；
  真机侧剩：RSSI/能量**实测标定**（F-11）、下行**重试/持久会话**（F-14 的 TLS 那部分，未做）。

## 10. 开放问题清单

| # | 问题 | 归属 | 状态 |
|---|---|---|---|
| F-01 | 身份标识（SN 码号）？ | §5 | **已定**：SN=`CC-ORG-UNIQUE[-CHECK]`（Crockford Base32），对齐《Orpah ID 协议规范》（SN 规则自其 v1.7 起定稿）；Server 校验非法回 FORMAT-ERR（2026-09-10 对齐） |
...
| F-11 | 免电池终端的能量预算与状态机（采集功率/储能/上报代价/**监听开销**的**真实标定**，以及“能量紧到什么程度该降级/该沉默”的**阈值**），是否要写进《Orpah ID 协议规范》§8.1 的“条件”列成为一条正式降级理由？ | §5.2/《Orpah ID 协议规范》§8.1/§9 | **部分已定（2026-09-13，含 E4 覆盖要求）**：机制与语义已定并实现（§5.2 的 E1/E2/E3/E4：降级可见、下限 L1、沉默归因分叉、**覆盖（不断线）+ 不够就降级换覆盖 + 断线前预警**；**监听作为固定开销进预算**，采集连监听都供不上时如实沉默、`short_of` 指明缺在哪一层；`energy.plan`/`coverage` + 告警 + UI）；**实测标定入口已实现**（`energy_calib.py`：JSON 文件 / `ORPAH_ENERGY_CALIB`，可写原始实测或 mW/mJ + **实测取能曲线** `harvest_curve`，逐项标 `measured`/`demo` + 溯源与文件指纹，**坏文件整份不采用**；阈值与**听间隔**等**策略项不是标定项**）；**待定 = 真机标定数值本身**（TX-AH+CH32V203 实测采集/待机/上报/听窗口/取能曲线）与是否把“能量”列入《Orpah ID 协议规范》§8.1 条件列 —— **没标定时一切仍为演示值（页面徽标会写“未标定”，不得当成实测）** |
...
| F-14 | **Router 的「下行」（Server→Router）没有真实性**（LOST-TABLE/回执/ERROR 均未签名）—— 怎么办？ | §8 威胁 4 | **✅ 已定（2026-09-13，用户选：先 A 后 B，均已完成）**。**A = 只接受配置的 Server 源 IP + 源端口**（`router._from_server`，解析之前就丢 + `down_rejected` 计数/留痕 + 页面可见；`server_host` 为主机名时先解析；**fail-closed**）。**B = Server 用私钥签、Router 只持公钥**：下行新增 `dts`/`dn`/`dsig`，预像 = `JCS(报文去掉 dsig)`；Router 要求验签通过 + `|now−dts| ≤ ORPAH_DOWN_WINDOW` + `(dts,dn)` 比上次新（**防重放，重放不需要私钥**）。**为什么不用 HMAC 共享密钥**：Router 本就假定可被改装，共享密钥进 Router = 把伪造能力交给改机的人；公钥不是秘密。边界：**空口那段（Router→Client）仍无认证**；Router 没配公钥时**计数 `down_unverified` 并显示“未启用”**（不假装已启用）；私钥保管（硬件信任根）与 TLS/持久会话（挡丢弃/延迟）**未做**。验收 `test_downlink.py` 44 项 + `test_router.py` + `test_server.py` + `demo_l4.py` 第 ⑨ 组（同源套接字伪造/重放均被丢） |

## 附：本文件填充路线

1. 定 §6（传输）→ 回填 §5 报文字节定义 → 补 §4 时序细节
2. 定 §7（走失表）与 §8（安全最小集）— **§8 已完成（2026-09-13）**：威胁 1/2/3 + 本 demo 实现现状 + 缺口，规范正文指向《Orpah ID 协议规范》v1.19
