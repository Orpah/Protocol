...
- 状态：**v0.7.5**（2026-09-13）· L1/L2 已在 `orpah-over-halow` 实现并验收
  ...
  **2026-09-13（本次）：§10 开放问题逐条结案** —— F-05/F-06/F-08 按已有实现落笔定稿，
  ...
  （规范正文在《Orpah ID 协议规范》v1.17，本文件不复制）；
...
- 缓解候选（**待定，先别实现**）：a) Router 侧身份与签名（Server 先认证 Router 再采信其观测）；
  b) 多 Router 交叉校验（同一时刻多台观测互证、离群剔除）；c) 只在受信集合内采信。
  - a)/c) 需要 Router 侧密钥与信任根 = **硬件信任根范畴**（`orpah-over-halow/ROADMAP.md` §〇 不做）；
  - b) 是**纯软件、可做**，但会改动「Server 把路由器当可信方」这一架构假设 →
    **动手前先与用户对齐**（可能超出 demo 边界）。
...
| F-12 | **路由器侧观测（`xport`）不在签名预像内** —— 能冒充/控制一台 Router 即可伪造定位（而 ORPAH 的前提正是「被改装的路由器」）。Server 要不要、以及如何建立「受信 Router 集合」？ | §8 / 《Orpah ID 协议规范》§5.7 | **开放（2026-09-13 记录）**：规格 §5.7 已给方向（Server **应校验 `router_bssid` ∈ 受信集合**），**demo 未实现**（无 Router 身份/白名单）；演示如实展示该边界（`spoof.py` 的 `xport_tamper` 期望值 = 通过）。缓解候选：a) Router 侧身份+签名（需信任根，按 §〇 不做）；b) 多 Router 交叉校验/离群剔除（**纯软件、可做**）；c) 只在受信集合内采信（同 a 的信任根问题）。**动手前与用户对齐**（可能超出 demo 边界） |
| F-13 | **密钥生命周期口径不一致**：《Orpah ID 协议规范》§6.3.1「**不实施密钥轮换**，一机一钥终身固定，只撤销」 vs demo 已实现**多代密钥 + 轮换 + grace + retired**（`keys.html` / `keystore.KeyStoreDB`，2026-09-12 P1）。以哪边为准？ | §8 / `orpah-over-halow` 实现 | **待拍板（2026-09-13 发现）**：两处口径必须统一 —— 要么改规格（承认轮换，v1.17 的「不做轮换」条作废），要么把 demo 的轮换降为「仅演示、不作真机模型」。**未拍板前不要按任一边继续加功能**（本表只记录，**两边都未改**） |
