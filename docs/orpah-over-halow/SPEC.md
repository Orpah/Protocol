...
- 状态：**v0.7.7**（2026-09-13）· L1/L2 已在 `orpah-over-halow` 实现并验收
  ...
  **同日晚续：一致判定算法落地** —— `orpah-over-halow/ui/static/pos.js` 的 `consensus()`
  （离线 13 条用例），判据定为 **σ0/χ²(0.99)** + **留一法 4σ**；实现里测出的两条硬边界已回写 §8 威胁 2
  （**不能用「单台残差 > k·σ」—— 会被整体拟合带弯、漏检**；**识别单台离群需 ≥4 台站位**）；
...
- **已定方向（2026-09-13 用户定，**取代**原「受信 Router 集合」建议；**同日已实现**于
  ...
  1. **判据 = 整体一致性**：后验单位权标准差 `sigma0 ≤ √(χ²(0.99)/dof)`（dof = 观测数 − 2）。
     **不要**用「单台残差 > k·σ_i」—— 单个离群观测会把**整体拟合带弯**，各台残差被摊平到各自
     阈值以内：实测 4 台里 1 台把距离报成 1/4，那一判据下**全部合格**（漏检）；σ0 是整体统计量、
     带不弯（同一份数据 2.30 > 上限 2.15 → 抓得住）。
  2. **找离群 = 留一法标准化残差** `z_i = |该台实测 − 其余台解出的位置应有的值| / σ_i`：取最大者，
     并要求 `z ≥ 4σ` **且剔掉它之后其余台真的自洽**才定案 → 剔除 + 作为**安全事件**记录
     （有攻击 = 有线索，不能静默丢弃）。实测分离度：离群台 z=12.0 vs 诚实台 2.98 / 2.03 / 0.45。
     （**为何不能只看「排除它后整体是否自洽」**：4 台剔到 3 台时 dof=1、χ² 上限很松，
     排除**诚实**那台也可能算出「自洽」→ 定不了谁。）
  3. **移动目标**：用滤波状态预测**各观测各自时刻**的位置再算残差（复用 `pos.js` 的恒速卡尔曼，
...
- **硬边界（必须如实遵守，不许包装成「防住了」）**：
  ...
  - **k 台合谋不可防**（信息论边界：剩 n−k 台，要 n−k ≥ 3 才还能定位）；
  - **能被抓的偏差下界 = 噪声的可分辨下界**：与测距噪声同量级的偏差原理上不可分（不报），
    这是老实话、不是漏检 —— 别为了「提高检出率」调低判据（那只会把正常噪声当攻击）。
