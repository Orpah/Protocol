...
- 状态：**v0.7.8**（2026-09-13）· L1/L2 已在 `orpah-over-halow` 实现并验收
  ...
  **同日晚续：一致判定算法落地** —— `orpah-over-halow/ui/static/pos.js` 的 `consensus()`
  （离线 **16** 条用例），判据定为 **留一法标准化残差**（σ **锚在其余台推出的距离上**，阈值 **3σ_rel**）；
...
- **已定方向（2026-09-13 用户定，**取代**原「受信 Router 集合」建议；**同日已实现**于
  ...
  1. **判据 = 留一法标准化残差** `z_i`：用**其余台**解出位置，再看这一台的实测距离"应该是多少"：
     ...
     **阈值 = 3σ_rel**（= 距离偏差达 75%）。两条实测踩出来的「**别这么写**」：
     ...
     - **σ 不能锚在它自己报的距离上**，必须锚在**其余台推出的**距离上：锚错时，谎报「我在更远处」
       ...
       在整条路线上 **0/120 全没发现**，还照样报「交叉校验通过」。锚对之后两边对称：×4 → 12.0、×0.25 → 3.0。
...
- **硬边界（必须如实遵守，不许包装成「防住了」）**：
  ...
  - **能被抓的偏差下界（对称）= 距离偏差 75%**：偏差在 ±75% 以内与 25% 的测距噪声
    **原理上不可分**（如实不报）—— 这是老实话、不是漏检；别为了「提高检出率」调低阈值
    （那只会把正常噪声当攻击：**假警报最害人**）。
