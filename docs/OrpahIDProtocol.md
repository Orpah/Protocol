...
因此任何单条 `xport` 都不可信**（v1.18 修正：原先建议的「server 应校验 `router_bssid` 是否属于
受信 router 集合」**已否决** —— 白名单只回答「谁在说」，不回答「说得对不对」，且信任根属硬件范畴）。

**规范要求（孤证不立 —— 本项目原则）**：

- **不得单独采信任一 `xport`**；粗定位结论必须来自**多个互不依赖的观测互相印证**
  （多观测一致性判定 + 离群剔除：用子集解位置 → 反推各 router 应有的 RSSI → 比较实测残差；
  残差离群者剔除，并作为**安全事件**上报 —— 有攻击 = 有线索，不得静默丢弃）。
- **只有 1~2 个观测时无冗余 → 不可判定**：实现方必须**如实降级**并标注「单一来源、未交叉校验」，
  **不得**当作可信结果输出。
- **k 个 router 合谋不可防**（信息论边界：剩余 n−k 个，需 n−k ≥ 3 才能定位）—— 实现方
  **不得**声称已防住；**可信度与定位精度必须分开呈现**（小误差圆 + 「单一来源」可以同时成立，
  而后者意味着不可信）。
- **移动目标**：残差必须在**各观测各自的时刻**上比较（用运动模型预测该时刻的位置），
  否则正常移动会被误判为篡改（**假警报最害人**）。
- **例外（唯一）**：证据**明确来自人**（**办案人 / 搜救队**）的单源证据**可采信**，但必须留痕
  （审计 `actor`）；人工来源**优先于**机器观测，且**不参与**一致性剔除。

> 口径同步：本条与 `docs/orpah-over-halow/SPEC.md` §8 原则 P-1 / §10 F-12 一致（2026-09-13）。
...
>
> **附：设备侧自愿自限频（2026-09-13，参考实现补充，不在本表四行之内）** —— 客户端也可以对自己的
> 上行限频（`ratelimit.DeviceLimiter`，`ORPAH_SELF_*`，默认最小间隔 0.6s / 突发 4）。它是**自愿**的：
> **不构成防线**（被改过或失控的设备不会做，本协议也要求不了它），语义是**延后（hold）而不是丢弃**
> —— 丢自己的业务报等于**漏报**。实际好处只有三个：不占满共享空口（帧被 Router 丢掉之前**已经上过空口**，
> 带宽已经花掉）、省电（免电池设备最紧的是每次上报能耗）、不撞上游的桶。默认参数按参考实现的真实节奏定、
> **同为部署配置**，真机需按空口与电池实测定。验收见 `orpah-over-halow` 的 `test_selflimit.py` +
> `demo_ratelimit.py` 第 ⑦ 组。
...
4. **本系统对使用者无强制约束力（2026-09-13 用户定）**：这不是金融/受监管系统 —— 人觉得不好用或不想用时，直接**抛弃客户端、或把它物理屏蔽（包锡纸等）**就行。既然"不想被追踪"只需物理屏蔽就能达到，就**不需要用轮换去对付"疑似泄露"**；轮换只会给愿意用的人制造失联风险。
5. **免电池/取能终端做不了轮换这种操作（2026-09-13 用户定）**：ORPAH 的终端设定是**免电池可穿戴**（靠运动/光/射频取能，见 §8.2 与能量轴），而轮换是"**高能耗 + 必须在线完成一次事务**"的操作（生成新钥 → 上行公钥 → 等 server 确认/绑定 → 由 server 指定切换时刻）。取能设备的采集功率是**波动的**，它**承诺不了"服务器要求换钥的那一刻我有电"** —— 这正是能量轴里 `P = 0`（采不敷出）的情形。宁可"密钥终身一把、能量只花在找人上"，也不要为了换钥把设备逼到沉默。

> **跟能量轴的关系**：设备侧的能量只用于**采集/上报**（与级别降级，见能量轴），不安排轮换事务。
> **跟 demo 的关系**：`orpah-over-halow` 里的 `rotate`/`grace`/`retired` 代码是**为了对比而留的演示/试验代码，不是设备模型**；
> 真机与本文本文档以【不轮换】为准，新代码不得基于轮换路径。
...
> **设备侧实际持有**：**1 把 ECDSA P-256 私钥**（Slot 0，终身）+ **1 把 32 字节 HMAC 密钥**（降级 HS256 用，Slot 5 / CH32 保护区）—— 共 2 份密钥材料、**一代**，不存在"第 2 代"。
...
| 1.18 | 2026-09-13 | 十审修正（**§5.7 定位数据源**）：删除「server 应校验 `router_bssid` 属于受信 router 集合」白名单建议（**已否决**：白名单只回答「谁在说」，不回答「说得对不对」）；改为**孤证不立**——单条 `xport` 永不单独采信、多观测一致性 + 离群剔除（离群者作安全事件上报）、无冗余（1~2 个观测）必须如实降级、**k 个 router 合谋不可防**、移动目标按各观测各自时刻比残差；**唯一例外** = 证据明确来自人（办案人/搜救队）单源可采信但必须留痕。与 `orpah-over-halow/SPEC.md` §8 原则 P-1 / §10 F-12 口径统一 |
| 1.19 | 2026-09-13 | 十一审（**§6.3.1 不轮换的理由补全**）：新增理由 4（**本系统对使用者无强制约束力** —— 觉得不好用/不想用，抛弃客户端或**物理屏蔽（包锡纸）**即可，因此不需要靠轮换应对"疑似泄露"）与理由 5（**免电池/取能终端做不了轮换** —— 轮换是"高能耗 + 必须在线完成一次事务"，取能设备承诺不了"换钥那一刻我有电"，即能量轴的 `P=0` 情形）；新增"设备侧实际持有"说明（**1 把 ECDSA P-256 + 1 把 32 字节 HMAC，一代，不存在第 2 代**）；并明确 demo 的 `rotate/grace/retired` 是**对比用的演示代码、不是设备模型**（同时结案 SPEC F-13） |
| 1.20 | 2026-09-13 | 十二审（**§5.8 非规范性补充，不改任何规范条款**）：在参考实现说明后加「设备侧自愿自限频」——`ratelimit.DeviceLimiter`（`ORPAH_SELF_*`，默认最小间隔 0.6s / 突发 4）。明确三点：① **它不在本表四行之内、不构成防线**（被改过或失控的设备不会做，本规范也要求不了它）；② 语义是**延后（hold）而不是丢弃**（丢自己的业务报 = **漏报**）；③ 好处只有三个（不占满共享空口、省电、不撞上游的桶），参数同为**部署配置**、未经真机标定。验收见 `orpah-over-halow` 的 `test_selflimit.py` + `demo_ratelimit.py` 第 ⑦ 组 |
